...
namespace ScopeAnalysis
{
    internal sealed class UartDecodeSettings
    {
        ...
        public int LeadingIdleSamples
        {
            get
            {
                return _leadingIdleSamples;
            }
            set
            {
                int normalizedValue = Math.Max(0, value);
                if (_leadingIdleSamples != normalizedValue)
                {
                    _leadingIdleSamples = normalizedValue;
                    _version++;
                }
            }
        }
        ...
        public int EmptyDataRunSegmentThreshold
        {
            get
            {
                return _emptyDataRunSegmentThreshold;
            }
            set
            {
                int normalizedValue = Math.Max(1, value);
